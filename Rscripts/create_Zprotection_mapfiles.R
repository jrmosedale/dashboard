...
###############################################################
### SET PROJECT PATH ###
dir_ZProject<-"D:/Zonation_Projects/Cornwall_Priority/"
...
new.r<-rank_after_mask(r,protected.r)
...
zmap<-leaflet() %>%
  ...
  addScaleBar() %>%
  addRasterImage(protected.r, colors = "yellow", opacity = 0.7,group="Protected areas") %>%
  addRasterImage(cws.r, colors = "purple", opacity = 0.5,group="County Wildlife Sites") %>%
  addRasterImage(zero_to_NA(priority.r), colors = "green", opacity = 0.5,group="Priority habitats") %>%
