...
# Aesthetics
aesthetics.r<-raster(paste0(in.root,"ecoservices/ecoservice_aesthetics.tif"))
...
# Soil carbon
soilcarbon.r<-mask(raster(paste0(in.root,"ecoservices/ecoservice_soilc.tif")),cornwall)
...
# Flood mitigation
floodmitig.r<-mask(raster(paste0(in.root,"ecoservices/ecoservice_floodmitig.tif")),cornwall)
crs(floodmitig.r)<- CRS('+init=EPSG:27700')
floodmitig.vals<-getValues(floodmitig.r)

# Drinking water quality
drinkqual.r<-mask(raster(paste0(in.root,"ecoservices/ecoservice_drinkwater.tif")),cornwall)
crs(drinkqual.r)<- CRS('+init=EPSG:27700')
drinkqual.vals<-getValues(drinkqual.r)

# Aquaculture water quality
aquaqual.r<-mask(raster(paste0(in.root,"ecoservices/ecoservice_aquaculture.tif")),cornwall)
crs(aquaqual.r)<- CRS('+init=EPSG:27700')
aquaqual.vals<-getValues(aquaqual.r)

# Bathing water quality 
bathqual.r<-mask(raster(paste0(in.root,"ecoservices/ecoservice_bathingwater.tif")),cornwall)
crs(bathqual.r)<- CRS('+init=EPSG:27700')
bathqual.vals<-getValues(bathqual.r)

# Not good catchments water quality 
catchqual.r<-mask(raster(paste0(in.root,"ecoservices/ecoservice_catchmentquality.tif")),cornwall)
...
leaflet() %>% 
  ...
  addRasterImage(aesthetics.r, colors=servicepal,
               opacity = 0.8,project=TRUE, group="Aesthetics") %>%
  ...
  hideGroup(c("Pollination","Soil loss mitigation",
             "Flood mitigation","Drinking water","Aquaculture","Bathing water", "Aesthetics"))
...
```{r,context='data', include=FALSE}
lightemission.r<-mask(raster(paste0(in.root,"ecoservices/light_emissions.tif")),cornwall)
crs(lightemission.r)<- CRS('+init=EPSG:27700')
lightem.vals<-getValues(lightemission.r)

lightskyglow.r<-mask(raster(paste0(in.root,"ecoservices/light_skyglow.tif")),cornwall)
